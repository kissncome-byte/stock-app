...
def unified_institutional_brain(res_dict, df_hist, is_holding=False, entry_cost=0.0, sector_panic=False):
    ...
    if is_holding and entry_cost > 0:
        ...
        if st_type == "RIGHT_BREAKOUT":
            ...
            # 🌟 【終極補救内鬼點】：無條件強勢歸位 272% 飆股突破最核心的 blueprint 通行證，擊殺 line 545 KeyError！
            return {
                ...
                "blueprint": {"停損防守": f"收盤實質跌破 {r:.2f} 元", "移動停利": f"動態守波動防線 ({trailing_stop:.2f} 元)", "預期目標": f"獲利擴張目標對位 {res_dict['target_brk']:.2f} 元"}
            }
...
if diag_trigger or stock_input:
    ...
    if res is None: st.error("該個股代碼數據獲取失敗。")
        ...
        # 區塊 B：三大法人買賣超
        st.markdown("### 🦅 三大法人每日實時進出買賣超佈局明細表 (近30日現況)")
