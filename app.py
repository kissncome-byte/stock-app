...
def unified_institutional_brain(res_dict, df_hist, is_holding=False, entry_cost=0.0, sector_panic=False):
    ...
    if is_holding and entry_cost > 0:
        ...
        if (abs(pnl_pct) <= 1.5) and (is_volume_gap_spike or is_rs_gold or (p >= r * 0.95)): return {"strategy_name": "🌱 新開倉動能蜜月期保護", "color": "#10B981", "action_now": "🟢 🟢 【全新部位：給予空間讓子彈飛】", "signal": "觸發防甩轎保護機制", "desc": f"您目前成本為 {entry_cost:.2f} 元（損益：{pnl_pct:+.2f}%）。風控模組已強制鎖定『蜜月保護盾』——自動幫你屏蔽短線減碼雜訊，嚴禁被洗下車！全額咬死現股，保險絲死守底線！", "blueprint": {"停損防守": f"核心資本死穴 {entry_cost * 0.93:.2f} 元", "移動停利": "新開倉開啟防甩轎保護", "預期目標": f"目標看擴張位 {res_dict['target_brk']:.2f} 元"}}
        ...
        # 🌟 【神經聯鎖重構】：讓緩衝帶格子自動解碼下方第一與第二面板的暗鬼因子，徹底打通因果鏈！
...
def evaluate_stock(stock_id: str, total_capital: float, risk_per_trade: float, slip_ticks: int, is_holding=False, entry_cost=0.0, sector_panic=False):
    ...
    # 🌟 【局部範疇防火牆】：將新聞抓取try-except完美定位在evaluate_stock體內，徹底解決528行死機
    try:
    ...
    if not fin_df_raw.empty and "Revenue" in fin_df_raw.columns and "EPS" in fin_df_raw.columns:
        ...
        gpm_now = safe_float(last_fin.get("gpm", 0.0))
        opm_now = safe_float(last_fin.get("opm", 0.0))
        sum_eps_4q = pd.to_numeric(fin_df_work.tail(4)['EPS'], errors='coerce').sum()
        ...
        if len(fin_df_work) >= 5:
            ...
            fin_conclusion = "⚖️ 財報常態運作中（歷史季度不足 5 季）"
    ...
    if "破底翻" in res_dict["tactical_blueprint"]["strategy_name"] and ("買點一成立" in spring_verdict or "買點二成立" in spring_verdict):
    ...
    else: strategy_route = "🚀 強勢突破前高劇本" if "突破" in res_dict["tactical_blueprint"]["strategy_name"] else "🛡️ 均線拉回低吸劇本"
