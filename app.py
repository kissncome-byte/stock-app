...
def _rq_get(url: str, headers=None, timeout=5):
    # 只用 certifi（你指定的 A）
    return requests.get(url, headers=headers or {"User-Agent": "Mozilla/5.0"}, timeout=timeout, verify=certifi.where())
...
with st.sidebar:
    ...
    run_screen = st.button("🚦 自動篩選候選股", type="secondary")
...
def fetch_twse_stock_day_all():
    ...
    js = r.json()
    df = pd.DataFrame(js)
    # 常見欄位: Code, Name, TradeVolume, TradeValue, OpeningPrice, HighestPrice, LowestPrice, ClosingPrice, Change, Transaction
...
def finmind_avg5_amount_yi(api: DataLoader, sid: str):
    # 取最近約 14 天，挑出最後 5 個交易日的 Trading_money 平均（億）
    ...
    if "Trading_money" not in df.columns:
        ...
        if "Trading_money " in df.columns:
            df = df.rename(columns={"Trading_money ": "Trading_money"})
        else:
            return np.nan, 0
...
if run_screen:
    ...
    with st.spinner("正在自動篩選（近5日平均成交金額）..."):
        try:
            ...
            if today_all is None or today_all.empty or "TradeValue" not in today_all.columns or "Code" not in today_all.columns:
                st.error("❌ 無法取得今日全市場成交資料（openapi.twse.com.tw）")
            else:
                topn = today_all.sort_values("TradeValue", ascending=False).head(int(screen_top_n)).copy()
                rows = []
                for _, r in topn.iterrows():
                    ...
                    rows.append({
                        ...
                        "n_days": n_days,
                        "today_trade_value_yi": float(r.get("TradeValue", 0.0)) / 1e8,
                        "close": safe_float(r.get("ClosingPrice", 0.0)),
                    })
