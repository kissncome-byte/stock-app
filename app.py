...
def prepare_indicator_df(df: pd.DataFrame):
    ...
    # 手動用 numpy 機制精算 DMI 方向指標
    up_diff = x["high"].diff()
    down_diff = x["low"].shift(1) - x["low"]
    x["P_DI"] = (np.where((up_diff > down_diff) & (up_diff > 0), up_diff, 0.0))
    x["M_DI"] = (np.where((down_diff > up_diff) & (down_diff > 0), down_diff, 0.0))
    tr_s = x["TR"].ewm(com=13, adjust=False).mean().replace(0, 0.00001)
    x["P_DI"] = (x["P_DI"].ewm(com=13, adjust=False).mean() / tr_s) * 100
    x["M_DI"] = (x["M_DI"].ewm(com=13, adjust=False).mean() / tr_s) * 100
...
def unified_institutional_brain(res_dict, df_hist, is_holding=False, entry_cost=0.0, sector_panic=False):
    ...
    if is_holding and entry_cost > 0:
        ...
        # 🌟 【終極神經因果聯鎖】：將第一（大盤）、第二（位階）面板的實時數據，100% 寫入量化緩衝帶的 desc 之中！
        neutral_desc = f"由於此時下方面板 1【{res_dict['market_vol_desc']}】，且面板 2 精算判定該股目前僅屬於【{wolf_rank_label}】，缺乏核心機構大金流點火表態。這兩大因子的空方排擠，導致大腦決策防線硬性退守至【量化緩衝帶】。此處毫無多頭期望值，強制空倉，手綁起來！"
...
def evaluate_stock(stock_id: str, total_capital: float, risk_per_trade: float, slip_ticks: int, is_holding=False, entry_cost=0.0, sector_panic=False):
    ...
    elif ma5_slope < -0.15: stable_short_trend, stable_short_color, stable_short_desc = "🔴 短期空頭修正（上方有壓，防禦觀望）", "#EF4444", "5日主力成本線集體下彎。短期多頭動能退潮，上方套牢怨魂沉重。即便盘中反彈，也切勿追高！"
