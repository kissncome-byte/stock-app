...
st.markdown("## 🧭 StockPilot Beta v9.6｜個股操作決策")
    ...
    if res is None:
        ...
        if not _p18:
            ...
            # v9.0：已持股完整操作中心
                ...
                # v9.6：出場比例直接使用畫面「訊號一致度」實際顯示的同一個計數。
                ...
                # v9.6：所有出場比例文字一律從同一組變數產生，禁止任何 UI 區塊另寫固定百分比。
                _exit_plan_text = (
                ...
                st.caption(
                    f"出場引擎依據：持股健康度 {_hold_health}｜"
                    f"訊號一致度 {_exit_signal_count}/{_hold_signal_total}"
                )
