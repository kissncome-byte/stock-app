...
def compute_live_data(stock_id: str, market_type: str, hist_last_close: float, hist_last_vol: float):
    ...
    for suffix in [".TWO", ".TW"] if is_otc_hint else [".TW", ".TWO"]:
        try:
...
def get_overnight_radar():
    ...
    radar_results, is_us_panic, panic_desc, wtx_change = {}, False, "", 0.0
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if result_list and len(result_list) > 0:
                        ...
                        current_price = closes[-1] if len(closes) >= 2 else safe_float(result_list[0].get("meta", {}).get("regularMarketPrice"))
                        prev_close = closes[-2] if len(closes) >= 2 else (safe_float(result_list[0].get("meta", {}).get("previousClose")) or safe_float(result_list[0].get("meta", {}).get("chartPreviousClose")))
                        
                        if prev_close > 0:
                            change_pct = ((current_price - prev_close) / prev_close) * 100
                            radar_results[label] = change_pct
                            if symbol == "WTX=F": wtx_change = change_pct
                            if symbol != "WTX=F" and change_pct <= -2.0:
                                is_us_panic, panic_desc = True, f"昨晚美股大震盪，{label} 慘跌 {change_pct:.1f}%"
                        break 
            except Exception: pass
...
def get_daily_df(stock_id: str, market_type: str = "TSE", days: int = 450): 
    ...
    start_dt = datetime.now(TZ) - timedelta(days=days)
    period1, period2 = int(start_dt.timestamp()), int(datetime.now(TZ).timestamp())
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if result_list and len(result_list) > 0:
                    ...
                    raw_df = pd.DataFrame({
                        "date": [datetime.fromtimestamp(ts, TZ).strftime("%Y-%m-%d") for ts in res_data.get("timestamp", [])],
                        "open": res_data.get("indicators", {}).get("quote", [{}])[0].get("open", []),
                        "high": res_data.get("indicators", {}).get("quote", [{}])[0].get("high", []),
                        "low": res_data.get("indicators", {}).get("quote", [{}])[0].get("low", []),
                        "close": res_data.get("indicators", {}).get("quote", [{}])[0].get("close", []),
                        "vol": res_data.get("indicators", {}).get("quote", [{}])[0].get("volume", []),
                    ...
                    raw_df["open"], raw_df["high"], raw_df["low"] = raw_df["open"]*raw_df["factor"], raw_df["high"]*raw_df["factor"], raw_df["low"]*raw_df["factor"]
                    raw_df["close"] = raw_df["adjclose"]
                    raw_df["amount"] = raw_df["close"] * raw_df["vol"]
                    return raw_df[["date", "open", "high", "low", "close", "vol", "amount"]].copy()
        except Exception: pass 
        
    try:
        df_raw = get_api().taiwan_stock_daily(stock_id=stock_id, start_date=start_dt.strftime("%Y-%m-%d"))
...
def get_market_macro_status():
    ...
    try:
        df = get_api().taiwan_stock_daily(stock_id="TAIEX", start_date=start_date)
        if df is not None and not df.empty:
            ...
            if is_waterfall_panic: return False, f"🚨 大盤瀑布重挫 ({last_row['close']:.1f})，近週跌 {market_5d_return:.1f}% 觸發【強勢股補跌危機】", True, False
            elif is_market_overextended: return True, f"⚠️ 大盤過熱警告 ({last_row['close']:.1f})，季線正乖離達 {bias_ma60:.1f}%【強制調降進場曝險】", False, True
            elif last_row['close'] >= last_row['MA20']: return True, f"加權指數 ({last_row['close']:.1f}) 站穩 20MA 多頭常態", False, False
            else: return False, f"加權指數 ({last_row['close']:.1f}) 跌破 20MA 空方警戒", False, False
    except Exception: pass
...
def get_taiwan_enhanced_chips(stock_id: str, days: int = 30):
    ...
    try:
        inst_df = get_api().taiwan_stock_institutional_investors(stock_id=stock_id, start_date=start_date)
    ...
    try:
        margin_df = get_api().taiwan_stock_margin_purchase_short_sale(stock_id=stock_id, start_date=start_date)
...
def get_financial_statement_df(stock_id: str, years: int = 2):
    try:
        df_raw = get_api().taiwan_stock_financial_statement(stock_id=stock_id, start_date=(datetime.now() - timedelta(days=years * 365)).strftime("%Y-%m-%d"))
        if df_raw is None or df_raw.empty: return pd.DataFrame()
        df = df_raw.copy()
        df["type"] = df["type"].replace({"OperatingRevenue": "Revenue"})
        df = df[df["type"].isin(["EPS", "Revenue", "GrossProfit", "OperatingIncome"])]
        return df.pivot_table(index="date", columns="type", values="value", aggfunc="last").reset_index()
...
def get_realtime_news_list(stock_id: str, stock_name: str):
    news_list = []
    try:
        query = urllib.parse.quote(f"{str(stock_name)} {str(stock_id)} when:1d")
        r = get_requests_session().get(f"https://news.google.com/rss/search?q={query}&hl=zh-TW&gl=TW&ceid=TW:zh-Hant", timeout=5)
        if r.status_code == 200:
            root = ET.fromstring(r.content)
            for item in root.findall('.//item'):
                title = item.find('title').text or ""
                if " - " in title: title = title.rsplit(" - ", 1)[0]
                news_list.append({"date": item.find('pubDate').text or "", "title": title, "source": item.find('source').text if item.find('source') is not None else "新聞財經", "link": item.find('link').text or ""})
            if news_list:
...
def prepare_indicator_df(df: pd.DataFrame):
    ...
    x["up_move"], x["down_move"] = x["high"].diff(), x["low"].shift(1) - x["low"]
    ...
    low_min, high_max = x["low"].rolling(9).min(), x["high"].rolling(9).max()
    x["RSV"] = 100 * ((x["close"] - low_min) / (high_max - low_min).replace(0, 0.00001))
    k_list, d_list, ck, cd = [], [], 50.0, 50.0
    for rsv in x["RSV"]:
        if pd.isna(rsv): k_list.append(np.nan); d_list.append(np.nan)
        else: ck = (2/3)*ck + (1/3)*rsv; cd = (2/3)*cd + (1/3)*ck; k_list.append(ck); d_list.append(cd)
    x["K9"], x["D9"] = k_list, d_list
    
    if "open" in x.columns:
        x["upper_shadow"] = x["high"] - np.maximum(x["open"], x["close"])
        x["is_long_upper_shadow"] = (x["upper_shadow"] > (x["open"] - x["close"]).abs()) & (x["upper_shadow"] / (x["high"] - x["low"]).replace(0, 0.00001) > 0.4)
...
def auto_strategy_classifier(res_dict):
    price, resistance, ma20_val, spring_verdict, trend_phase = res_dict["current_price"], res_dict["real_resistance"], res_dict["ma20_val"], res_dict["spring_verdict"], res_dict["trend_phase"]
    if "買點一成立" in spring_verdict or "買點二成立" in spring_verdict or "破底翻結構醞釀中" in spring_verdict:
        if price < resistance * 0.98: return "LEFT_SPRING", "🛡️ 左側交易：破底翻良性換手型態"
    if price >= resistance * 0.97 or (price > ma20_val and trend_phase == "🔥 波段多頭主升段"): return "RIGHT_BREAKOUT", "🚀 右側交易：強勢多頭突破型態"
...
def unified_institutional_brain(res_dict, df_hist):
    ...
    price, resistance, ma20_val, ma100_val, sitc_3d = res_dict["current_price"], res_dict["real_resistance"], res_dict["ma20_val"], res_dict["ma100_val"], res_dict["sitc_3d_sum"]
    macro_safe = "安全" in res_dict["macro_desc"] or "站穩" in res_dict["macro_desc"] or "過熱" in res_dict["macro_desc"]
    is_market_panic, is_market_overextended = res_dict.get("is_market_panic", False), res_dict.get("is_market_overextended", False)
    is_us_panic, us_panic_desc, wtx_change = res_dict.get("is_us_panic", False), res_dict.get("us_panic_desc", ""), res_dict.get("wtx_change", 0.0)
    is_wtx_panic = wtx_change <= -1.0
    final_decision, atr = res_dict["final_decision"], res_dict["atr"]
    ...
    if "長上影" in final_decision or "金流陷阱" in final_decision or (is_kd_dead_cross and is_high_risk_zone):
        verdict_msg = "❌ 爆量長上影：大戶資金出貨賣壓沉重！" if "長上影" in final_decision else "🚨 惡性金流陷阱：隔日沖主力開高出貨踩踏！" if "金流陷阱" in final_decision else "⚠️ 超買區高檔死亡交叉！主力技術性調節動能弱化。"
        return {"strategy_name": strategy_name, "color": "#FF4B4B", "action_now": "🚨 🔴 【立即清倉 / 獲利了結】", "signal": "極端出貨與慣性改變訊號共振", "desc": verdict_msg, "blueprint": {"停損防守": "無（全面轉入清倉程序）", "移動停利": "無", "預期目標": "資金退場保全"}}

    if (is_wtx_panic or is_us_panic) and strategy_type == "RIGHT_BREAKOUT":
        return {"strategy_name": strategy_name, "color": "#F59E0B", "action_now": "⚠️ 🟡 【夜盤背離：沒收開火權觀望】", "signal": "🚨 跨市場金流斷層：期現貨嚴重背離", "desc": f"昨晚台指期夜盤暴跌 {wtx_change:.2f}% 或美股重挫（{us_panic_desc}），早盤個股強行突破極高機率為主力拉高出貨盤。強行沒收追高開火權，封鎖曝險！", "blueprint": {"停損防守": "嚴禁進場", "移動停利": "無", "預期目標": "等待賣壓消化"}}

    if is_wtx_panic and strategy_type == "LEFT_SPRING":
        return {"strategy_name": strategy_name, "color": "#EF4444", "action_now": "🛑 🔴 【期現貨跳空引信：取消低吸掛單】", "signal": "📉 夜盤引力崩塌：左側均線全面失效", "desc": f"夜盤實質重挫 {wtx_change:.2f}%，今日現貨開盤加權指數將大幅跳空低開，強勢股必然面臨補跌踩踏，嚴禁伸手接飛刀！", "blueprint": {"停損防守": "禁止接刀", "移動停利": "無", "預期目標": "靜待止穩"}}

    if is_market_panic and strategy_type != "RIGHT_BREAKOUT":
        return {"strategy_name": strategy_name, "color": "#EF4444", "action_now": "🛑 🔴 【強勢股補跌警戒：關閉低吸掛單】", "signal": "☠️ 總體流動性清算：多頭踩踏進行中", "desc": "加權指數5日內跌逾3.5%且失守月線，全網觸發非自願性基金贖回與融資踩踏，強勢股將發生補跌瀑布，硬性關閉所有低吸劇本！", "blueprint": {"停損防守": "禁止開火", "移動停利": "無", "預期目標": "保留寶貴大資金"}}

    if not macro_safe:
        return {"strategy_name": strategy_name, "color": "#FF4B4B", "action_now": "🚨 🔴 【強制空倉防禦 / 嚴禁開火】", "signal": "大盤空頭暴風雨警戒", "desc": "加權指數跌破20MA生命線，大環境架構轉弱，強勢股突破極易淪為拉高誘多出貨點，無條件一票否決！", "blueprint": {"停損防守": "禁止進場", "移動停利": "觀望", "預期目標": "等待大盤重返安全區"}}
    ...
    if strategy_type == "RIGHT_BREAKOUT":
        if macro_safe and price >= ma100_val and price >= resistance * 0.99 and res_dict["vol_spike"] and sitc_3d > 300 and f_is_good and c_is_locked:
            if is_market_overextended:
                return {"strategy_name": strategy_name, "color": "#F59E0B", "action_now": "⚠️ 🟡 【大盤過熱：防守型控量輕倉開火】", "signal": "⚡ 瘋狗浪末段逆勢突破：慎防高檔流動性陷阱", "desc": "個股觸發五維度黃金共振！但加權指數與季線正乖離率已突破過熱天花板(>8.5%)，追高極易撞上假突破，解鎖開火權但底層硬性扣減60%資金曝險配額！", "blueprint": {"停損防守": f"收盤跌破前高牆 {resistance:.2f} 元", "移動停利": f"盤中跌破動態 ATR 安全線 {brk_trailing_stop:.2f} 元", "預期目標": f"獲利飽和對位目標看 {res_dict['target_brk']:.2f} 元"}}
            return {"strategy_name": strategy_name, "color": "#7D3CFF", "action_now": "🔮 🔮 【立即開火進場】", "signal": "🔮 頂級多頭共振：黃金主升飆股型態發動", "desc": "五維度指標完美共振！大盤多頭護航，財報基本面強勢擴張，疊加法人鎖碼與放量突破前高牆，上方無套牢怨魂，大膽切入！", "blueprint": {"停損防守": f"收盤跌破支撐牆 {resistance:.2f} 元", "移動停利": f"即時價跌破動態回撤線 {brk_trailing_stop:.2f} 元", "預期目標": f"第一階段波段擴獲利看 {res_dict['target_brk']:.2f} 元"}}
        
        if price < brk_trailing_stop:
            return {"strategy_name": strategy_name, "color": "#FF4B4B", "action_now": "⚠️ 🔴 【動態多頭防線破防：分批落袋】", "signal": "右側動能高位衰竭回撤", "desc": f"股價已跌破動態波動率最高價移動安全防線 ({brk_trailing_stop:.2f} 元)，微觀動能急凍，判定轉入防守分批獲利程序。", "blueprint": {"停損防守": "無", "移動停利": "已觸發", "預期目標": "鎖住利潤資金騰出"}}
        
        if t_is_strong and f_is_good:
            return {"strategy_name": strategy_name, "color": "#1C86EE", "action_now": "🔥 🔵 【穩健波段主升：持股續抱】", "signal": "多方有序推進、基本面實質支撐", "desc": "短長期趨勢維持健康多頭排列，基本面結構扎實，大戶籌碼無異常流失，有部位者現股續抱，讓利潤在主升浪中奔跑。", "blueprint": {"停損防守": f"技術硬風控底線 {res_dict['stop_brk']:.2f} 元", "移動停利": f"動態 ATR 防守點為 {brk_trailing_stop:.2f} 元", "預期目標": f"波段目標價持續對位 {res_dict['target_brk']:.2f} 元"}}

    elif strategy_type == "LEFT_SPRING":
        if is_market_overextended:
            return {"strategy_name": strategy_name, "color": "#64748B", "action_now": "⚖️ 🔵 【大盤過熱：關閉左側抄底掛單】", "signal": "📥 總體估值亢奮：拒絕左側逆張交易", "desc": "大盤正乖離率處於極端超買歷史高位，隨時面臨多翻空修正威脅。此時任何個股左側抄底均易被砸成真破底，一票否決！", "blueprint": {"停損防守": "禁止進場", "移動停利": "無", "預期目標": "等待大盤乖離率修正完畢"}}
        if "買點一成立" in res_dict["spring_verdict"] or "買點二成立" in res_dict["spring_verdict"]:
            return {"strategy_name": strategy_name, "color": "#10B981", "action_now": "🟢 🟢 【立即精密低吸進場】", "signal": "🛡️ 良性回檔：最完美破底翻結構確立", "desc": f"{res_dict['spring_verdict']} 估值回踩至具備安全邊邊際之低位，散戶浮額被洗淨，主力洗盤誘空完成，此進場點死穴極小，享有極致風險報酬比！", "blueprint": {"停損防守": f"硬性死穴防線對位洗盤最低點 {recent_lowest:.2f} 元", "移動停利": "左側不採用移動停利", "預期目標": f"定點停利直指前高壓力牆 {res_dict['target_pb']:.2f} 元"}}
        if price < recent_lowest:
            return {"strategy_name": strategy_name, "color": "#FF4B4B", "action_now": "🛑 🔴 【立即現股砍單停損】", "signal": "左側結構崩毀、轉惡性真破底", "desc": f"最新市價已崩跌破近十日洗盤實質最低點 ({recent_lowest:.2f} 元)，假跌破被無情證偽，轉為真破底，請立刻砍單停損離場！", "blueprint": {"停損防守": "已觸發死穴（立刻執行）", "移動停利": "無", "預期目標": "保全資金實力"}}
        return {"strategy_name": strategy_name, "color": "#1C86EE", "action_now": "⚖️ 🔵 【空倉保持耐心 / 靜待右腳確認】", "signal": "假破底洗盤完成、多頭翻轉訊號醞釀中", "desc": "體質符合良性回檔，融資大退。但盤中微觀訊號尚未確認放量翻轉發動，提早建倉時間成本高昂，請保持耐心等待綠燈訊號。", "blueprint": {"停損防守": f"預估防守硬線為近十日低點 {recent_lowest:.2f} 元", "移動停利": "無", "預期目標": f"翻轉確立反彈目標看 {res_dict['target_pb']:.2f} 元"}}
    return {"strategy_name": strategy_name, "color": "#1C86EE", "action_now": "⚖️ 🔵 【遵循量化紀律常規操作】", "signal": "後台因子互有勝負、未達極端背離", "desc": "財務與動能因子未觸發極端背離共振，個股處於箱型調整區，請嚴格遵循下方精密雙軌量化藍圖精算之價位執行紀律操作。", "blueprint": {"停損防守": f"突破防守線 {res_dict['stop_brk']:.2f} 元 ｜ 低吸防守線 {res_dict['stop_pb']:.2f} 元", "移動停利": "暫不啟動動態停利線", "預期目標": f"突破目標看 {res_dict['target_brk']:.2f} 元 ｜ 低吸反彈目標看 {res_dict['target_pb']:.2f} 元"}}
...
def evaluate_stock(stock_id: str, total_capital: float, risk_per_trade: float, slip_ticks: int):
    ...
    # 🌟 徹底解鎖：完美接通 prepare_indicator_df 函數與變數 Scope 的閉環
    ...
    if is_compressed and (df["close"].tail(10) < df["MA20"].tail(10)).sum() >= 7: bb_stage = "💤 打底觀望期：布林帶收窄壓縮，股價長時運行於中軌下方。主力低檔吸籌【只觀察、絕不進場】"
    elif current_price > ma20_val and df["close"].iloc[-2] <= df["MA20"].iloc[-2] and df["close"].iloc[-1] > df["open"].iloc[-1]: bb_stage = "🔥 啟漲共振點：一根紅陽實體強勢突破藍色 MA20 中軌並收穩，趨勢正式由空轉多！"
    elif current_price >= bb_upper or (current_price > ma20_val and df["close"].tail(5).mean() > bb_upper * 0.95): bb_stage = "🚀 主升維持階段：強勢多頭沿布林黃色上軌持續推升，直至股價遠離軌道進入調整"
    ...
    if k9_now < 20 and d9_now < 20: kd_timing = "📥 打底階段：KD 指標回落至 20 以下超賣區，屬於典型超跌蓄力狀態，靜待共振反彈"
    elif "啟漲" in bb_stage and k9_now >= 45 and is_kd_had_low_cross_recently: kd_timing = "⚡ 共振啟漲點：KD 先於低位金叉蓄勢落底，今日股價突破布林中軌時順勢衝破 50 多空分水嶺共振發動！"
    elif k9_now > 70: kd_timing = "🚨 高檔死亡交叉：超買區反轉弱化訊號觸發，短線多頭調節" if (df["K9"].iloc[-1] < df["D9"].iloc[-1]) and (df["K9"].iloc[-2] >= df["D9"].iloc[-2]) else "🦅 高檔判定：數值衝破 70 進入超買區，強勢主升浪出現長時間鈍化，無需盲目賣出"

    if "打底" in bb_stage or (df["vol"].tail(10) < vol_ma20_val).sum() >= 7: volume_verdict = "📉 底部震盪期：成交量長期萎縮，代表浮動籌碼已被主力高度鎖定"
    elif vol_spike and ("啟漲" in bb_stage or current_price >= real_resistance * 0.95): volume_verdict = "🐳 共振突破點：突破關鍵位階且紅量柱連續堆高，遠高於均量，大資金真金白銀進場！"
    elif (df["close"].iloc[-1] > df["close"].tail(15).max() * 0.98) and ((current_vol * 1000.0) < vol_ma20_val * 0.8):
        volume_verdict = "🦅 強力鎖碼縮量主升：大腦啟動主力豁免檢查！法人與特定大戶高度控盤鎖死籌碼（浮額洗淨），此為空氣單無量主升段，續抱讓利潤奔跑！" if main_force_score >= 65 or "強力鎖碼" in sitc_trend else "🚨 散戶型量價背離風險：後續股價創新高但量能萎縮，屬於虛胖的量價背離，多頭動能衰退"
    ...
    if spring_triggered:
        if current_price >= detected_prior_low and df["close"].iloc[-2] <= detected_prior_low and df["close"].iloc[-1] > df["open"].iloc[-1]: spring_verdict = f"🟢 【破底翻：買點一成立】主力砸盤誘空完成！股價放量收陽線重新站回前低 {detected_prior_low:.2f} 元。第一安全切入點觸發，輕倉試布局！"
        elif current_price >= detected_neckline and vol_spike: spring_verdict = f"🔮 【破底翻：買點二成立】多頭翻轉爆發！股價放量強勢突破關鍵頸線 {detected_neckline:.2f} 元。確認開啟波段上漲，追加倉位！"
        else: spring_verdict = f"🔍 【破底翻結構醞釀中】觸發經典假破底洗盤（前低：{detected_prior_low:.2f}，關鍵頸線：{detected_neckline:.2f}），正等待多頭翻轉訊號。"

    kd_status = "黃金交叉" if k9_now > d9_now else "死亡交叉"
    # 🌟 語法更正補丁：將遺留下的 JS 語法 && 完美更正為 Python 的 and 語法，排除編譯器紅字
    ...
    if not fin_df.empty and "Revenue" in fin_df.columns and "EPS" in fin_df.columns:
        ...
        if sum_eps_4q > 0:
            ...
            dynamic_bubble_threshold = 55.0 if latest_yoy >= 30.0 else 45.0 if latest_yoy >= 15.0 else 35.0
            dynamic_cheap_threshold = 22.0 if latest_yoy >= 30.0 else 18.0 if latest_yoy >= 15.0 else 13.0
        ...
        if len(fin_df) >= 5:
            ...
            gpm_text = "優於去年" if gpm_now > safe_float(prev_fin.get("gpm", 0.0)) else "遜於去年"
            opm_text = "優於去年" if opm_now > safe_float(prev_fin.get("opm", 0.0)) else "遜於去年"
    ...
    package = {
        ...
        "vol_spike": vol_spike, "pe_desc": pe_desc, "margin_trend": margin_trend, "target_brk": round_to_tick(current_price + ((4.0 if df["amount"].tail(20).mean() > 2000000000 else 5.5) * atr), t),
        ...
        "target_pb": round_to_tick(real_resistance, t), "stop_pb": round_to_tick(ma20_val - atr - (float(slip_ticks) * t), t) if round_to_tick(ma20_val - atr - (float(slip_ticks) * t), t) < current_price else round_to_tick(current_price - (1.5 * atr), t),
    ...
    if "破底翻" in tactical_blueprint["strategy_name"] and ("買點一成立" in spring_verdict or "買點二成立" in spring_verdict):
        tactical_blueprint["desc"] = f"{spring_verdict} 此型態屬於主力經典的誘空套路：砸破低點營造崩盤恐慌，再快速閃電拉回。技術停損點已硬性對位在洗盤實質最低點 {spring_lowest_low:.2f} 元。"
    ...
    return {
        ...
        "target_brk": package["target_brk"], "stop_brk": package["stop_brk"], "rr1_brk": rr1_brk, "target_pb": package["target_pb"], "stop_pb": package["stop_pb"], "rr1_pb": rr1_pb,
        "suggested_lots": suggested_lots, "expected_stop_price": expected_stop_price, "strategy_route": strategy_route, "expected_target_price": package["target_brk"] if "突破" in tactical_blueprint["strategy_name"] or "暫緩追高" in tactical_blueprint["action_now"] else package["target_pb"], 
...
if diag_trigger or (not scan_trigger and stock_input):
    with st.spinner("五維度大腦深度因果解耦中..."):
        ...
        if res is None: st.error("該個股代碼數據獲取失敗，請確認編號是否正確（如：新上市或未滿100交易日股票數據不足）。")
        else:
            bp_data = res["tactical_blueprint"]
            ...
            with bl1: st.markdown(f"""<div style="background-color: #F8FAFC; padding: 16px; border-radius: 6px; border-left: 5px solid #2563EB; border-top: 1px solid #E2E8F0; border-right: 1px solid #E2E8F0; border-bottom: 1px solid #E2E8F0;"><h4 style="margin: 0 0 12px 0; color: #1E40AF; font-weight:800;">🚀 流派一：突破前高起漲劇本 (Breakout)</h4><p style="font-size: 14px; margin: 5px 0;"><b>精密建倉觸發點</b>：&le; {res['real_resistance']:.2f} 元</p><p style="font-size: 14px; margin: 5px 0;"><b>期望波段獲利目標</b>：<span style="color:#2563EB; font-weight:700;">{res['target_brk']:.2f} 元</span></p><p style="font-size: 14px; margin: 5px 0;"><b>技術硬性防守停損</b>：{res['stop_brk']:.2f} 元</p><p style="font-size: 14px; margin: 5px 0;"><b>期望風險報酬比 (R:R)</b>：{res['rr1_brk']:.2f}</p></div>""", unsafe_allow_html=True)
            with bl2: st.markdown(f"""<div style="background-color: #F8FAFC; padding: 16px; border-radius: 6px; border-left: 5px solid #10B981; border-top: 1px solid #E2E8F0; border-right: 1px solid #E2E8F0; border-bottom: 1px solid #E2E8F0;"><h4 style="margin: 0 0 12px 0; color: #065F46; font-weight:800;">🛡️ 流派二：均線拉回低吸劇本 (Pullback)</h4><p style="font-size: 14px; margin: 5px 0;"><b>精密低吸型買點</b>：貼近 {res['ma20_val']:.2f} 元</p><p style="font-size: 14px; margin: 5px 0;"><b>期望反彈獲利目標</b>：<span style="color:#10B981; font-weight:700;">{res['target_pb']:.2f} 元</span></p><p style="font-size: 14px; margin: 5px 0;"><b>技術硬性防守停損</b>：{res['stop_pb']:.2f} 元</p><p style="font-size: 14px; margin: 5px 0;"><b>期望風險報酬比 (R:R)</b>：{res['rr1_pb']:.2f}</p></div>""", unsafe_allow_html=True)

            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown("### 🛡️ 量化核心風控配額開火劇本")
            if res["suggested_lots"] == 0:
                if "#FF4B4B" in bp_data["color"] or "#EF4444" in bp_data["color"] or "#F59E0B" in bp_data["color"]: st.error("🚨 【核心風控最高警戒：大腦策略拒絕進場】 敞口強制關閉！大盤或夜盤觸發系統性恐慌/背離倒貨風險。")
                else: st.warning("⚠️ 【風控提示：資金配額不足 1 張】 當前標的趨勢健康，但因資金池較小或風險承受％數太嚴格。系統為保護帳戶不給予強行開火建議。")
